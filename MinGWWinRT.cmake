...
option(MINGW_WINRT_DISABLE_CORE_LIBS "Do not link WinRT core import libs (windowsapp, runtimeobject)" OFF)
if(MINGW_WINRT_UNCAGED)
  set(MINGW_WINRT_DISABLE_CORE_LIBS ON CACHE BOOL "Do not link WinRT core import libs (windowsapp, runtimeobject)" FORCE)
  set(MINGW_WINRT_USE_WINSTORECOMPAT OFF CACHE BOOL "Link winstorecompat shim when available" FORCE)
endif()
...
function(_mingw_winrt__try_sdk_version _out_ok _root _ver _arch)
  ...
  if(NOT MINGW_WINRT_DISABLE_CORE_LIBS)
  ...
  if(MINGW_WINRT_DISABLE_CORE_LIBS)
    set_property(GLOBAL PROPERTY MINGW_WINRT_LIB_WINDOWSAPP   "")
    set_property(GLOBAL PROPERTY MINGW_WINRT_LIB_RUNTIMEOBJECT "")
  else()
...
function(_mingw_winrt__locate_winstorecompat _out_path)
  ...
  if(DEFINED ENV{MSYSTEM_PREFIX} AND NOT "$ENV{MSYSTEM_PREFIX}" STREQUAL "")
    list(APPEND _paths "$ENV{MSYSTEM_PREFIX}/lib")
  endif()
  if(DEFINED CMAKE_FIND_ROOT_PATH)
    foreach(_r IN LISTS CMAKE_FIND_ROOT_PATH)
      if(_r)
        list(APPEND _paths "${_r}/lib")
      endif()
    endforeach()
  endif()
  if(DEFINED CMAKE_CXX_IMPLICIT_LINK_DIRECTORIES)
  ...
  set(_lib "")
  foreach(_d IN LISTS _paths)
    if(NOT _lib AND EXISTS "${_d}/libwinstorecompat.a")
      set(_lib "${_d}/libwinstorecompat.a")
    elseif(NOT _lib AND EXISTS "${_d}/winstorecompat.lib")
      set(_lib "${_d}/winstorecompat.lib")
    endif()
  endforeach()
  if(NOT _lib)
    find_library(_lib NAMES winstorecompat PATHS ${_paths} NO_DEFAULT_PATH)
  endif()
  if(NOT _lib)
...
function(_mingw_winrt__validate_toolchain)
  ...
  if(NOT (CMAKE_CXX_COMPILER_ID STREQUAL "GNU" OR CMAKE_CXX_COMPILER_ID STREQUAL "Clang"))
    message(FATAL_ERROR "MINGW_USE_WINRT=ON requires a MinGW-compatible compiler (GNU or Clang). Current compiler: ${CMAKE_CXX_COMPILER_ID}")
  endif()
  if(NOT WIN32)
    message(FATAL_ERROR "MINGW_USE_WINRT=ON requires targeting Windows (CMAKE_SYSTEM_NAME=Windows).")
  endif()
  if(NOT MINGW)
    message(STATUS "MinGW WinRT: 'MINGW' variable not set, but compiler is GNU/Clang + target is WIN32; continuing.")
  endif()
endfunction()
...
function(_mingw_winrt__configure_once)
  ...
  set(_winstorecompat "")
  if(MINGW_WINRT_USE_WINSTORECOMPAT AND NOT MINGW_WINRT_DISABLE_CORE_LIBS)
